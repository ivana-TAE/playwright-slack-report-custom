...
test.describe('ResultsParser', () => {
    ...
    test('parseTests calculates effective retries correctly for flaky tests', async ({}) => {
        ...
        (0, test_1.expect)(testResults[0].retries).toBe(1); // Failed attempt with effective retries
        (0, test_1.expect)(testResults[0].retry).toBe(0);
        (0, test_1.expect)(testResults[0].status).toBe('failed');
        (0, test_1.expect)(testResults[1].retries).toBe(1); // Passed attempt with effective retries
        (0, test_1.expect)(testResults[1].retry).toBe(1);
        (0, test_1.expect)(testResults[1].status).toBe('passed');
    });
    test('parseTests handles single failure correctly', async ({}) => {
        ...
        (0, test_1.expect)(testResults[0].retries).toBe(0);
        (0, test_1.expect)(testResults[0].retry).toBe(0);
        (0, test_1.expect)(testResults[0].status).toBe('failed');
    });
    ...
    test('getFailures excludes flaky tests correctly with fix', async ({}) => {
        ...
        (0, test_1.expect)(failures[0].test).toBe('Failed Test [chrome]');
        (0, test_1.expect)(failures[0].suite).toBe('Failed Suite');
        (0, test_1.expect)(failures[0].failureReason).toContain('Test failed permanently');
    });
    ...
    test('getFailures correctly handles many flaky tests with single real failure', async ({}) => {
        ...
        (0, test_1.expect)(failures).toHaveLength(1);
        (0, test_1.expect)(failures[0].test).toBe('Permanently Failed Test [safari]');
        (0, test_1.expect)(failures[0].suite).toBe('Real Failure Suite');
        (0, test_1.expect)(failures[0].failureReason).toContain('This test always fails permanently');
        ...
        (0, test_1.expect)(flakyTest1).toBeUndefined();
        (0, test_1.expect)(flakyTest2).toBeUndefined();
        (0, test_1.expect)(flakyTest3).toBeUndefined();
    });
});
