...
test_1.test.describe('CLI app - pre-check', () => {
    ...
    (0, test_1.test)('throws an error when both sendUsingWebhook and sendCustomBlocksInThreadAfterIndex are truthy', async ({}) => {
        const invalidConfig = path_1.default.join(__dirname, 'test_data', 'invalid_cli_config_sendCustomBlocksInThreadAfterIndex_enable_for_webhook.json');
        const result = await (0, cli_pre_checks_1.default)(validTestResults, invalidConfig);
        (0, test_1.expect)(result.status).toEqual('error');
        (0, test_1.expect)(result.message).toContain('The sendCustomBlocksInThreadAfterIndex feature is only supported when using sendUsingBot is configured');
    });
